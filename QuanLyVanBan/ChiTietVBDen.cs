...
{
	public partial class ChiTietVBDen : Form
	{
		public ChiTietVBDen(DataGridViewRow row)
		{
			InitializeComponent();
			int i = 0;
			...
			lbTacGia.Text = row.Cells[i++].Value.ToString();
			lbSoTrang.Text = row.Cells[i++].Value.ToString();
			lbSoBan.Text = row.Cells[i++].Value.ToString();
			lbChuyenCA.Text = row.Cells[i++].Value.ToString();
